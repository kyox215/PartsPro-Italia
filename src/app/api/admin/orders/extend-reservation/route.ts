import { adminExtendOrderReservation } from "@/admin/services/order-mutations";
import {
  adminAccessError,
  adminCsrfError,
...
export async function POST(request: Request) {
  ...
  const result = await adminExtendOrderReservation(parsed.data, {
    request,
    actor: admin.context,
  ...
  return respondAdminMutation({
    ...
    backUrl,
    result,
    successCode: result.ok && "demoMode" in result.data ? "demo" : "extended",
  });
}

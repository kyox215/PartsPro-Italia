...
import { submitAccountOrderPaymentProof } from "@/lib/account-workflow";
import { parseRequestBody } from "@/lib/request";
import { hasSupabaseAdminConfig } from "@/lib/supabase/admin";
import { accountOrderPaymentProofSchema } from "@/lib/validations";

export const runtime = "nodejs";

export async function POST(request: Request) {
  const rawBody = await parseRequestBody(request);
  const parsed = accountOrderPaymentProofSchema.safeParse(rawBody);
  ...
  const redirectUrl = new URL(
    `/${locale}/account/orders/${encodeURIComponent(orderId)}`,
    request.url,
  );
  ...
  try {
    ...
    redirectUrl.searchParams.set(
      "error",
      error instanceof Error ? error.message : "payment_proof_failed",
    );
  }

...
import { recordAccountOrderMessage } from "@/lib/account-workflow";
import { parseRequestBody } from "@/lib/request";
import { hasSupabaseAdminConfig } from "@/lib/supabase/admin";
import { accountMessageSchema } from "@/lib/validations";

export const runtime = "nodejs";

export async function POST(request: Request) {
  const rawBody = await parseRequestBody(request);
  const parsed = accountMessageSchema.safeParse(rawBody);
  ...
  try {
    ...
    redirectUrl.searchParams.set(
      "error",
      error instanceof Error ? error.message : "message_failed",
    );
  }
